...
- Fix `torch.compile` support for `torch.combinations` by removing a CUDA synchronization from argument validation (#189305)
- Match eager CUDA behavior for `torch.addmm(..., beta=0)` by ignoring a non-broadcastable bias while retaining dtype and device validation (#183511)
- Fix ordering, dependency, and stream-state races in compiled multi-stream graphs that use events or the `control_deps` operator (#183803, #183804, #186022, #186023, #186025)
- Fix `_scaled_mm` compilation when tensorwise scales mix scalar and singleton two-dimensional shapes (#183964)
- Fix Inductor pattern-matching failures when `torch.randperm` is followed by advanced or sliced indexing (#184066)
- Prevent incorrect convolution binary folding when the convolution bias is a dynamic graph input (#184132)
- Prevent incorrect results when fusing user-defined Triton kernels with epilogues whose read and write layouts differ by rejecting the incompatible fusion (#184248)
- Fix `CantSplit` compilation errors when split iteration ranges contain equivalent symbolic floor-division expressions (#184566)
- Fix `torch.remainder` returning results that differ from eager mode when a `float16` or `bfloat16` tensor is combined with a Python or zero-dimensional scalar divisor (#185168)
- Fix FlexAttention illegal-memory-access failures on very large key/value buffers by using 64-bit pointer offsets when required (#185264)
- Fix CPU C++ code generation for vectorized atomic addition with scalar destination indices (#185325)
- Fix `"cannot determine truth value of Relational"` failures for dynamic-shape adaptive average pooling and `Upsample(mode="area")` (#185369)
- Fix `KeyError: 'complex64'` during Triton signature generation when compiled graphs require exact-stride copies of complex tensors (#185501)
- Stop successful CUDA compilation from emitting TF32 `UserWarning` messages while retaining the advisory under `TORCH_LOGS=perf_hints` (#185541)
- Fix assertion failures when lowering custom operators with out variants and dynamic output shapes (#185601)
- Preserve the graph's fake-tensor mode when `standalone_compile` receives tensor-subclass outputs (#185638)
- Fix an import-time `TypeError` in `torch._inductor` when another vendored `typing_extensions` copy has modified `typing` internals (#185708)
- Fix CPU max-autotune `IndexError` failures when a GEMM and its epilogue reuse the same input tensor (#185767)
- Prevent `RecursionError: maximum recursion depth exceeded` during AOTInductor autotuning when very wide symbolic size expressions are emitted into Triton source (#185778)
- Fix `torch.cond` compilation when a branch uses a tensor constant that was not registered in the root graph (#185838)
- Fix silently incorrect CPU results when outer-loop fusion reuses local buffers across tiled iterations (#185855)
- Fix generated regional-compilation wrappers referencing undefined symbols when an input dimension is a symbolic expression (#185890)
- Fix compiled `torch.min` and `torch.max` returning incorrect indices for boolean tensors on CPU and other non-Triton backends (#185970)
- Prevent `UnicodeDecodeError` while probing MSVC when `cl.exe /help` emits malformed or locale-specific bytes (#185972)
- Raise a clear error when FlexAttention backward receives a `score_mod` whose output does not depend on `score`, instead of failing inside Inductor lowering (#185991)
- Prevent fused kernels with large constant address offsets from overflowing 32-bit indices and raising out-of-range errors (#186060)
- Prevent `NoValidChoicesError` for Triton-only transposed or dilated convolution autotuning by supporting `ConvTranspose2d` and falling back safely when needed (#186067)
- Fix wrong results from Halide fusion when a producer reads a buffer that an in-place mutation also writes (#186121)
- Fix AOTInductor integer floor division by captured tensor constants, including correct rounding for negative values (#186242)
- Prevent duplicate equivalent template or external-kernel registrations from crashing imports with an assertion (#186262)
- Fall back to eager execution instead of failing compilation for `_scaled_mm_v2` with swizzled scale layouts such as blockwise MXFP8 and NVFP4 (#186384)
- Prevent NVGEMM precompilation from initializing CUDA inside an incompatible forked compile worker (#186385)
- Fix CPU code generation for `index_put_(accumulate=True)` with boolean values (#186523)
- Allow eager pointwise `associative_scan` to use its generic fallback on devices without scan code generation, and report unsupported compiled devices during Inductor lowering with an actionable error suggesting `combine_mode="generic"` (#186633)
- Fix CUTLASS kernel compilation through the C++ wrapper on XPU by using the XPU compiler and code cache (#186791)
- Match eager CPU and MPS semantics for `float16` and `bfloat16` addition and subtraction with Python scalar operands (#186818)
- Honor user-supplied FlexAttention kernel options and report incompatible sparse-block and tile sizes with actionable errors (#186876)
- Fall back to regular loads and stores when tensor descriptors cannot satisfy Triton's 16-byte transfer requirement, including `XBLOCK=1` and small persistent-reduction cases (#186922, #186932)
- Preserve eager signed-zero and NaN behavior for compiled `torch.minimum` and `torch.maximum` on Triton backends (#186933)
- Fix incorrect CPU results for `abs()` on unsigned integer tensors under `torch.compile` (#187024)
- Fall back to an unfused epilogue instead of raising `AssertionError: failed to set ranges` when a template's tiling cannot represent its iteration domain (#187209)
- Fix a race when an input requiring alignment correction is consumed on multiple CUDA streams by emitting one copy per stream (#187224)
- Fix combo-kernel compilation for persistent reductions with dynamic reduction dimensions (#187275)
- Make compiled `torch.celu` and `torch.celu_` reject `alpha=0` with the same error as eager execution (#187321)
- Prevent manual collective bucketing from producing a non-topological FX graph when packing order differs from graph order (#187341)
- Match eager CUDA behavior by returning `NaN` for infinite inputs to compiled modified-Bessel I0 and I1 operations (#187354)
- Fix `Unregistered range symbol` failures for persistent-TMA `addmm` kernels with fused epilogues (#187371)
- Avoid CUDA convolution-backward illegal memory accesses by falling back from Triton to the ATen implementation (#187372)
- Fix CUTLASS GEMM epilogue-fusion crashes when reshaped external inputs have incompatible dimensions (#187404)
- Fix `RuntimeError: 0 active drivers` in asynchronous Triton compile workers after CUDA has already been initialized (#187408)
- Fall back to NVML for GPU clock-rate queries when `nvidia-smi` is unavailable (#187427)
- Preserve NaN and infinity semantics by not folding floating-point or complex multiplication by zero into a constant zero (#187580)
- Prevent generated Triton cache paths from exceeding Windows path-length limits by truncating long fused-kernel names while retaining a uniqueness hash (#187641)
- Prevent memory planning from destroying an allocation pool too early when a graph output aliases a reused buffer (#187678)
- Fix `torch._inductor` import failures on Python 3.11 and 3.12 caused by supplying too few generic parameters to `CSE` (#187700)
- Fix sliced FlexAttention `BlockMask` objects retaining the original query sequence length instead of the sliced length (#187886)
- Preserve architecture suffixes such as `sm_100a` when packaging AOTInductor CUDA kernels, avoiding `ptxas fatal: PTX with .target 'sm_100a' cannot be compiled for architecture 'sm_100'` (#187888)
- Prevent duplicate Metal kernel names when a compiled graph contains multiple MPS scheduling instances (#187894)
- Fix FlexAttention illegal memory accesses when `score_mod` or `mask_mod` captures tensors requiring 64-bit indexing (#187904)
- Preserve the sign of `float64` negative zero in compiled CUDA `torch.signbit` (#187941)
- Match eager results for floating-point floor division with infinite operands and signed-zero quotients (#188049)
- Add an opt-in decomposition of DTensor shard-dimension all-to-all operations into traceable functional collectives through `TORCHINDUCTOR_DECOMPOSE_SHARD_DIM_ALLTOALL=1`. The same change always registers an autograd formula for `_dtensor::shard_dim_alltoall`, preventing compiled backward graphs from silently dropping its gradient even when the decomposition is disabled (#188137)
- Prevent the CUTLASS backend from generating unsupported INT8 matrix-multiplication kernels on SM 10.3 GPUs (#188209)
- Make eager and compiled `torch.cummax` and `torch.cummin` validate invalid dimensions consistently for zero-element tensors instead of silently returning empty results (#188361)
- Report CUDA Graph skip reasons on supported non-CUDA GPU backends instead of silently recording only a counter (#188384)
- Fix `LoweringException: AssertionError` when compiling scatter operations with empty index tensors (#188466)
- Fix FlexAttention's FLASH backend ignoring sparse `BlockMask` metadata when `mask_mod` is trivial (#188484)
- Fix wrong results in compiled programs that use bidirectional custom-stream synchronization (#188533)
- Match eager CUDA behavior by returning `NaN` for infinite inputs to compiled Bessel J0, J1, Y0, and Y1 operations (#188556)
- Fix CUDA `misaligned address` failures when a cached fast launcher outlives its autotuned kernel module (#188607)
- Fix `cudaErrorStreamCaptureUnsupported` when `torch.linalg.eigh` is compiled with CUDA Graphs enabled (#188641)
- Fix backward compilation failures caused by autograd-generated `aten.expand` calls carrying the schema's `implicit` keyword (#188758)
- Fix severely incorrect nested reduction results by rewinding block pointers across outer loops (#188771)
- Fix `ValueError: The argument 'False' is not comparable` when compiling boolean `torch.minimum` or `torch.maximum` (#188862)
- Fix NVGEMM compilation against newer CuTeDSL releases that removed the enum-based `fence_proxy` API (#188865)
- Fix FlexAttention FLASH-backend compilation and incorrect gathers caused by mixed 32-bit and 64-bit captured-buffer indices (#188876)
- Release benchmark buffers retained by failed Triton autotuning configurations, preventing large transient GPU-memory leaks (#188907)
- Preserve user stream assignments when compiler pattern replacements introduce intermediate operations (#189095)
- Preserve eager ordering for captured CUDA event recording, waits, and CPU-blocking stream synchronization (#189096)
- Fix silent miscompiles and illegal memory accesses when exact index expressions overflow 32-bit addressing (#189108)
- Cache dynamically expanded reduction configurations so later processes do not retune near-tied kernels and select inconsistent performance or numerics (#189124)
- Read kernel templates as UTF-8 to prevent locale-dependent `UnicodeDecodeError` compilation failures (#189196)
- Fix scheduler `KeyError` failures when mutation propagation creates a self-referential buffer dependency (#189288)
- Prevent `torch.compile` from hanging indefinitely when an Inductor compile-worker subprocess dies or shuts down unexpectedly (#189290)
- Fix severe `torch.erfc` and `torch.special.gammaincc` tail inaccuracies on MPS in eager and compiled execution (#189291)
- Fix compiled RNG-consuming fallback operators on XPU by using accelerator-neutral default-generator handling (#189310)
- Fix invalid generated Python for user-defined Triton kernels whose metadata contains `Enum` values by serializing each enum's underlying value (#189494)
- Fix XPU compilation of quantized tensor-subclass weights by skipping shared-input linear fusion only when `torch.cat` is unsupported (#189509)
- Fix `NameError: name 'u0' is not defined` when compiling `torch.cond` branches whose outputs differ in an inner dimension (#189529)
- Fix lowering of `float8_e4m3fn` inputs that use byte-backed Triton storage on CUDA devices older than SM 8.9 (#189561)
- Fix `NotImplementedError: make_reindexer NYI on DtypeView(...)` when compiling dtype-bitcast workloads such as MXFP8 scaling (#189584)
- Avoid wrong results and illegal memory accesses from compiled CUDA `ConvTranspose2d` by using the ATen implementation (#189660)
- Prevent result corruption when memory planning reuses storage returned by mutating fallback operators (#189735)
- Prevent NVGEMM compilation failures by rejecting kernels that do not support the exact target GPU architecture (#189775)
- Fix a compilation hang caused by reentrant NVGEMM epilogue-kernel cache lookup (#189780)
- Fix CUDA Graph capture failures for NVGEMM `addmm` epilogues whose outputs are intermediate tensors (#189781)
- Fix `RuntimeError: Graph output must be a ()` when Inductor rewrites a non-tuple FX graph output into a tuple (#189887)
- Fix `Unregistered range symbol` failures after fused Triton epilogues rename their iteration index (#189890)
- Fix silently wrong results when a padded tensor is fused into a split reduction and its masked load is served from the common-subexpression store cache (#189896)
- Fix silently incorrect gradients in compiled convolution, pooling, and `torch.where` graphs when scheduler recomputation loses mutation dependencies (#185873)
- Fix `NoValidChoicesError` for dynamic-shape ROCm matrix multiplication by using ROCm's `origami` GEMM performance model only when dimensions are statically known (#190024)
- Fix AOTInductor constant-graph code generation when runtime constant folding and lazy autotuning are enabled together (#190073)
- Fix FX-wrapper code generation for aliased fallback outputs that have no unbacked-symbol bindings (#190255)
- Preserve an explicit `dtype=` when Inductor replaces constant `torch.cumsum` patterns (#190328)
- Reject unsound loop-index inversions that could generate incorrect indexing expressions during fusion (#190401)
- Fall back to backend code generation instead of raising `OverflowError` when index propagation casts `inf` or `NaN` to an integer dtype (#190427)
- Fix cross-warp read-after-write races after stores inside reduction loops by inserting a barrier before reading the buffer (#190519)
- Match eager behavior for `AdaptiveAvgPool2d(0)` with `int64` inputs by returning an empty output instead of rejecting the dtype (#190531)
- Fix severe precision loss for small values of CPU-vectorized `torch.expm1` by using the vectorized `expm1` intrinsic instead of computing `exp(x) - 1` (#190533)
- Fix `LoweringException: 'float' object has no attribute 'get_dtype'` for floating-point floor division with a Python-scalar dividend (#190566)
- Fix dynamic-shape compilation failures caused by reciprocal powers in symbolic stride guards (#190965)
- Fix incorrect or nondeterministic results for indexing expressions whose surviving modular-index terms may be negative (#190966)
- Preserve positional arguments to operations such as `torch.clamp` during pre-gradient batch-math fusion (#190976)
- Fix C++ compilation failures when Python library search paths contain spaces (#191010)
- Fix incorrect results from compiled small matrix multiplications with transposed operands (#191127)
- Fix compiled `int32` and `int64` addition, subtraction, and multiplication on CPU to preserve PyTorch wrapping semantics on overflow (#191132)
- Fix compile failures when a fused scaled-dot-product-attention pattern incorrectly matches unsupported 3D key permutations (#191260)
- Stop repeated `TypedStorage is deprecated` warnings while Inductor sizes autotuning outputs (#191383)
- Fix CPU max-autotune batched-matmul compilation when a GEMM output is both returned and consumed by another operation (#191502)
- Fall back from tiling analysis instead of raising `InductorError: AssertionError` for nested modular-indexing or floor-division expressions (#191605)
- Fix `uniform_` on non-contiguous views producing NaNs or out-of-range values under `torch.compile` (#191709)
- Honor `torch.use_deterministic_algorithms()` for compiled scans such as CUDA `cumsum` (#191714)
- Fix failures in Inductor's FX wrapper backend when a `torch.cond` branch captures a compound symbolic integer such as `2*s0 + 1` (#191811)
- Fix an out-of-bounds vectorized `atomic_add` in CPU code generation that could abort on x86 or silently produce wrong results on aarch64 (#191861)
- Fix silently wrong results in TMA mix-order reductions where the x offset was not advanced between loop iterations (#192344)
- Prevent the CUTLASS backend from generating unsupported INT8 UMMA kernels on SM 10.7 GPUs (#192414)
- Fix out-of-bounds `IndexError` failures during `addmm` autotuning when a symbolic row count is hinted as zero (#192553)
- Avoid races while loading saved compiler-cache artifacts by populating bundled Triton kernels on demand through the JIT path instead of eagerly; cold cache loads may compile kernels on first use (#192526)
